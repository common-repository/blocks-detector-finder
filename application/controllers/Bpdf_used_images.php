...
class Bpdf_used_images extends Winter_MVC_Controller {
    ...
    public function index()
	{
        ...
        foreach ( $posts as $key => $page ) {
            ...
            if(isset($elementor_data[0]))
            {
                ...
                if(!is_array($elementor_data) || !is_string($elementor_data[0]))
                {
                    ...
                    {
                        echo '<div class="notice notice-warning"><p>'._('Parsing elementor data issue on page:').' <a href="'.get_admin_url().'post.php?post='.esc_url($page->ID).'&action=edit">#'.esc_html($page->ID).', '.esc_html($page->post_title).'</a></p></div>';
                ...
                foreach($matches[1] as $match)
                {
                    if(strpos($match, 'wp-content') !== FALSE)
                    {
                        ...
                        || strpos($img_filename, ' ') !== FALSE || strtolower($img_filename) != $img_filename)
                        ...
                        if(file_exists($img_path))
                        {
                            ...
                            if($width > 1280 || $height > 1280)
                                $red_messages[] = 'Large image dimension > 1280';
        ...
        // Load view
        $this->load->view('bpdf_used_images/index', $this->data);
    }
